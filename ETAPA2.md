...
| 5 | Módulo transporte de personal minero (Cat. 7) | ✅ **Implementado** — panel admin → "Transporte Cat. 7": modos con factor editable (kgCO2e/pasajero-km) y viajes que cargan la cuenta de carbono del Capital Natural. También desde el panel del proveedor (pestaña "Transporte Cat. 7"): la empresa registra sus propios traslados con evidencia opcional (PDF/XML/foto), puede precargar origen/destino/patente subiendo el XML de una guía de despacho (POST /leer-guia, migración 096 — la patente es informativa, no participa del cálculo de CO2e), y genera su informe mensual consolidado en PDF — mismos modos, factores y cálculo que el admin (migración 090, routes/transporteProveedor.js). |
