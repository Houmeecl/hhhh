...
import { CUENTAS_BASE, TIPOS_CUENTA, hashAsiento, validarLineas } from '../services/contabilidad.js';
...
async function balance(cliente_id, periodo_id) {
  const { rows } = await query(
    `SELECT c.id, c.codigo, c.nombre, c.tipo, c.activa,
...
router.post('/cuentas/base', puedeEditar, async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `INSERT INTO contabilidad_cuentas (cliente_id,codigo,nombre,tipo)
       SELECT $1, x.codigo, x.nombre, x.tipo FROM unnest($2::text[], $3::text[], $4::text[]) AS x(codigo,nombre,tipo)
       ON CONFLICT (cliente_id,codigo) DO NOTHING RETURNING *`,
      [id, CUENTAS_BASE.map((c) => c[0]), CUENTAS_BASE.map((c) => c[1]), CUENTAS_BASE.map((c) => c[2])]
    );
...
router.post('/cuentas', puedeEditar, async (req, res, next) => {
  try {
    ...
    if (!codigo || !nombre || !TIPOS_CUENTA.includes(tipo)) return res.status(400).json({ error: 'Código, nombre y tipo de cuenta válidos son obligatorios.' });
    const { rows } = await query('INSERT INTO contabilidad_cuentas (cliente_id,codigo,nombre,tipo) VALUES ($1,$2,$3,$4) RETURNING *', [id, String(codigo).trim(), String(nombre).trim(), tipo]);
