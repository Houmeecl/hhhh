...
test('GET /api/torre/flota sin token → 401', async () => {
  const res = await fetch(`${baseUrl}/api/torre/flota`);
  assert.equal(res.status, 401);
});

test('GET /api/torre/flota con rol incorrecto (tarjeta, no pos) → 403', async () => {
...
test('GET /api/torre/flota refleja los camiones creados por demo-torre', async () => {
...
test('POST /api/tarjeta/paso con punto_id fuera del catálogo: se guarda igual, con advertencia', async () => {
  const resAuth = await fetch(`${baseUrl}/api/tarjeta/auth`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
...
test('POST /api/tarjeta/paso con punto_id del catálogo real: sin advertencia de punto', async () => {
  const resAuth = await fetch(`${baseUrl}/api/tarjeta/auth`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ serial: camionEnRuta.tarjeta.serial, clave: camionEnRuta.tarjeta.clave }),
  });
...
test('POST /api/torre/mensaje con destino inválido → 400', async () => {
...
test('POST /api/torre/mensaje con frontera y zona fuera de los 3 pasos conocidos → 400', async () => {
  const res = await fetch(`${baseUrl}/api/torre/mensaje`, {
    method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${torreToken}` },
...
test('POST /api/torre/mensaje con estacionamiento y zona → 201', async () => {
  const res = await fetch(`${baseUrl}/api/torre/mensaje`, {
    method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${torreToken}` },
...
test('POST /api/torre/mensaje con rol incorrecto (tarjeta, no pos) → 403', async () => {
  const resAuth = await fetch(`${baseUrl}/api/tarjeta/auth`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ serial: camionEnRuta.tarjeta.serial, clave: camionEnRuta.tarjeta.clave }),
  });
...
test('POST /api/torre/mensaje sin token → 401', async () => {
  const res = await fetch(`${baseUrl}/api/torre/mensaje`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ codigo_lote: camionEnRuta.lote.codigo, destino: 'puerto' }),
  });
