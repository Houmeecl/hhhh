...
| Cobro real por correo (campañas) | **HECHO** (migración 100 + `services/cobros.js`, `services/pagos.js`, `services/tabla.js`, `routes/cobros.js`, sección admin `cobros`): se importa una lista de empresas desde Excel/CSV —con lector propio de .xlsx sin dependencias nuevas: ZIP + `zlib.inflateRawSync` + XML—, a cada una le llega su link de pago y, al confirmarse el pago, el sistema emite un `codigos_acceso` con créditos y se lo manda por correo SIN intervención humana. Dos pasarelas tras la misma interfaz: `manual` (transferencia bancaria, funciona hoy sin contratar nada) y **Flow.cl** (`PAGOS_PASARELA=flow`) — NO VirtualPos, que era el plan anterior para el terminal físico descontinuado. La regla que sostiene todo: **el webhook de Flow no se cree** — solo se lee su `token` y el estado real se pregunta de vuelta a Flow con la consulta firmada, y el monto se compara contra el cobrado antes de entregar nada; la entrega es idempotente (`SELECT ... FOR UPDATE` + `codigo_id`), así que dos avisos del mismo pago emiten UN código. Estados `pagado` → `entregado` separados a propósito: el correo con la clave puede fallar después de que el dinero entró, y el panel lo muestra como pendiente de reenvío. Cumplimiento: todo correo comercial lleva enlace de baja y cabeceras `List-Unsubscribe` de un clic (RFC 8058), la lista de bajas (`bajas_correo`) se respeta en todas las campañas y no se purga nunca, y hay bitácora de envíos (`correos_enviados`). El envío va por tandas con tope (`COBROS_LOTE_MAX`) para no quemar la reputación del dominio del que también salen los correos de activación. | Credenciales de Flow (`FLOW_API_KEY`/`FLOW_SECRET`) o datos bancarios (`PAGO_*`) en el `.env`. Antes de una campaña masiva: SPF, DKIM y DMARC del dominio en regla. |
